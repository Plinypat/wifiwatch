/**
 * WiFi Watch Observatory — Main Scene Orchestrator
 *
...
class Observatory {
  constructor() {
    ...
    // Load saved settings
    try {
      const ver = localStorage.getItem('wifi-watch-settings-version');
      if (ver === SETTINGS_VERSION) {
        const saved = localStorage.getItem('wifi-watch-observatory-settings');
        if (saved) Object.assign(this.settings, JSON.parse(saved));
      } else {
        localStorage.removeItem('wifi-watch-observatory-settings');
        localStorage.setItem('wifi-watch-settings-version', SETTINGS_VERSION);
      }
    } catch {}
